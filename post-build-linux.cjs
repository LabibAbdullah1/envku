...
async function main() {
  console.log("\n🚀 Envku Linux Post-Build Script (AppImage & DEB)");
  ...
  if (fs.existsSync(DEB_BUNDLE_DIR)) {
    ...
    debTarGzFile = debFiles.find(f => f.toLowerCase().startsWith("envku_") && f.endsWith(".deb.tar.gz"));
    debSigFile = debFiles.find(f => f.toLowerCase().startsWith("envku_") && f.endsWith(".deb.tar.gz.sig"));
    if (debFile) {
  ...
  // ── Salin file updater DEB .tar.gz & .sig jika ada ───────────
  if (debTarGzFile && debSigFile) {
    console.log(`📦 Menyalin arsip updater DEB (.deb.tar.gz) dan signature (.sig)...`);
    fs.copyFileSync(path.join(DEB_BUNDLE_DIR, debTarGzFile), path.join(DOWNLOADS, debTarGzFile));
    fs.copyFileSync(path.join(DEB_BUNDLE_DIR, debSigFile), path.join(DOWNLOADS, debSigFile));
    console.log(`   ✅ Berhasil disalin.`);
  } else {
    console.warn(`⚠️  Arsip updater DEB atau file signature tidak ditemukan di folder bundle.`);
  }
  ...
  if (fs.existsSync(UPDATE_JSON)) {
    ...
    if (tarGzFile && sigFile) {
      ...
      console.log(`   ✅ update.json diperbarui untuk linux-x86_64.`);
    }

    if (debTarGzFile && debSigFile) {
      const debSigContent = fs.readFileSync(path.join(DOWNLOADS, debSigFile), "utf8").trim();
      updateJson.platforms["linux-x86_64-deb"] = {
        signature: debSigContent,
        url: `https://envku.subly.my.id/downloads/${debTarGzFile}`
      };
      console.log(`   ✅ update.json diperbarui untuk linux-x86_64-deb.`);
    }
